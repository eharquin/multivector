...
describe('standard VGA 2D interpretation', () => {
  it('maps a pure grade-one owned value to a semantic vector', () => {
    expect(interpretVga2(ownedMultivector([0, 2, 1, 0], VGA_2D_BASIS))).toEqual({
      kind: 'vector-2d',
      x: 2,
      y: 1,
      approximated: false,
  ...
  it('classifies a value spanning odd and even grades as mixed', () => {
    expect(interpretVga2(ownedMultivector([1, 2, 1, 0], VGA_2D_BASIS))).toEqual({
      kind: 'mixed-multivector',
    })
  })

  it('classifies the all-zero multivector canonically as scalar zero', () => {
    expect(interpretVga2(ownedMultivector([0, 0, 0, 0], VGA_2D_BASIS))).toEqual({
      kind: 'scalar',
      value: 0,
      approximated: false,
    })
  ...
  it('maps a nonzero grade-zero value to a scalar entity', () => {
    expect(interpretVga2(ownedMultivector([12, 0, 0, 0], VGA_2D_BASIS))).toEqual({
      kind: 'scalar',
      value: 12,
      approximated: false,
    })
  ...
  it('maps a pure grade-two value to a semantic bivector', () => {
    expect(interpretVga2(ownedMultivector([0, 0, 0, -3], VGA_2D_BASIS))).toEqual({
      kind: 'bivector-2d',
      value: -3,
      approximated: false,
    })
  ...
  it('maps a non-pure even value to a semantic rotor', () => {
    expect(interpretVga2(ownedMultivector([1, 0, 0, -1], VGA_2D_BASIS))).toEqual({
      kind: 'rotor-2d',
      scalar: 1,
      bivector: -1,
      approximated: false,
  ...
  describe('tolerance-aware classification boundaries', () => {
    it('keeps a deliberate small literal above the floor as its own kind', () => {
      expect(interpretVga2(ownedMultivector([0, 0, 0, 5e-9], VGA_2D_BASIS))).toEqual({
        kind: 'bivector-2d',
        value: 5e-9,
        approximated: false,
      })
    ...
    it('suppresses a deliberate small literal below the absolute floor', () => {
      expect(interpretVga2(ownedMultivector([0, 0, 0, 5e-11], VGA_2D_BASIS))).toEqual({
        kind: 'scalar',
        value: 0,
        approximated: true,
      })
    ...
    it('ignores dominant-grade leakage beside a large coefficient', () => {
      ...
      expect(interpretVga2(ownedMultivector([-1, 0, 0, 3.6e-9], VGA_2D_BASIS))).toEqual({
        kind: 'scalar',
        value: -1,
        approximated: true,
      })
    ...
    it('keeps a standalone near-zero result at its own grade', () => {
      ...
      expect(interpretVga2(ownedMultivector([0, 0, 0, 1.7e-9], VGA_2D_BASIS))).toEqual({
        kind: 'bivector-2d',
        value: 1.7e-9,
        approximated: false,
      })
    ...
    it('scales suppression with magnitude via the relative term', () => {
      expect(interpretVga2(ownedMultivector([1000, 0, 0, 2e-5], VGA_2D_BASIS))).toEqual({
        kind: 'scalar',
        value: 1000,
        approximated: true,
      })
      expect(interpretVga2(ownedMultivector([0.001, 0, 0, 2e-11], VGA_2D_BASIS))).toEqual({
        kind: 'scalar',
        value: 0.001,
        approximated: true,
      })
